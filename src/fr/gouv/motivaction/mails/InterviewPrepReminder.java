...
public class InterviewPrepReminder extends AlertMail {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException
    {
        ...
        MailService.sendMailReport(Utils.concatArrayString(MailTools.tabEmailIntra, MailTools.tabEmailDev, MailTools.tabEmailExtra), "Rapport " + MailTools.env + " - Allez-vous être recruté ?", body);
    }
    ...
    private String buildAndSendInterviewPrepReminder(long userId)
    {
        ...
        try
        {
        	// @RG - EMAIL : la campagne de mail "Allez-vous être recruté" s'adresse aux utilisateurs notifiables ayant une candidature non archivée avec un entretien prévu dans 48h et pour laquelle il n'y a pas déjà eu un evt de type 'AI_PREPARE'
    ...
    private void sendInterviewPrepReminder(UserSummary user, String nomCandidature, String nomSociete, Timestamp eventTime, boolean test)
    {
        String subject = "Allez-vous être recruté";
        ...
        html += "Malgré vos expériences et votre motivation pour le poste, un candidat moins compétent, mais mieux préparé, aura plus de chances que vous...<br /><br />" +
                "<ul><li>Quelles sont les expériences que vous allez valoriser ?</li>" +
                "<li>Rassurez moi, vous avez bien prévu les questions qui montrent votre intérêt pour l'entreprise ?</li>" +
                "<li>...et partir suffisamment en avance pour ne pas arriver en retard, c'est ok pour vous ? ;-)</li>" +
                "</ul><br />Découvrez 2 sites pour préparer parfaitement votre entretien : <ul>" +
                "<li><a href='http://www.emploi-store.fr/portail/services/bABaEntretien?utm_campaign=memo&utm_medium=email&utm_source=prepReminder'>B.A.-BA Entretien</a></li>" +
                "<li><a href='http://www.emploi-store.fr/portail/services/monEntretienDEmbauche?utm_campaign=memo&utm_medium=email&utm_source=prepReminder'>Mon entretien d'embauche (simulateur)</a></li></ul>" +
