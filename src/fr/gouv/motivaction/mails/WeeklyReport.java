...
public class WeeklyReport extends AlertMail {
	...
	static {
		...
		tabConseil[2] = "Les offres d'emploi ne représentent qu'1/3 des opportunités d'emploi !<br/><br/>" +
						...
						"Vous pouvez enregistrer sur MEMO toutes les entreprises qui vous intéressent en un clic : c'est facile, il vous suffit de cliquer sur le bouton \"Importer sur MEMO\" disponible sur chaque entreprise.<br/><br/>" +
		...
		tabConseil[5] = "Sur MEMO, les offres d'emploi qui ne sont plus en ligne sont tout de même conservées dans votre tableau de bord et automatiquement signalées. C'est peut-être le bon moment pour effectuer une dernière relance ou bien pour les supprimer !<br/><br/>";
    ...
    public String buildAndSendWeeklyTaskReminder(long userId)
    {
        ...
        {
            if (currentCandidature!=null && currentCandidature.getId()!=0) {
            	log.error(logCode + "-004 Error processing weekly reminder email candidature. userId=" + userId + " error=" + e + " cand=" + currentCandidature.getId()+"-"+currentCandidature.getNomCandidature());
            }
            else
                log.error(logCode + "-005 Error processing weekly reminder email candidature. userId=" + userId + " error=" + e);
            //e.printStackTrace();
            err++;
        }
        finally
