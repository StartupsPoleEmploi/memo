...
public class CandidatureService {
    ...
    public static String translateURLIndeed(String u)
    {
        ...
        // forme attendue https://www.indeed.fr/m/viewjob?jk=52db384e8c6c7791
        // forme en entrée https://www.indeed.fr/jobs?q=informaticien&l=Paris%20(75)&vjk=d429e7c15ab1251a
        // forme en entrée https://www.indeed.fr/m/viewjob?jk=52db384e8c6c7791&from=serp&prevUrl=https%3A%2F%2Fwww.indeed.fr%2Fm%2Fjobs%3Fq%3Dsecretaire%2Bmedicale%26vjk%3D6d6ffd9bdc5a83df%26l%3DVilleurbanne%2B%252869%2529

        if(u.indexOf("vjk=")>=0)
        {
            idOffre = u.substring(u.indexOf("vjk=")+4);
            if (idOffre.indexOf("&")!=-1)
                idOffre = idOffre.substring(0, idOffre.indexOf("&"));

            res += idOffre;
        }
        else    // version mobile déjà bien calibrée
            res = u;
    ...
    public static String getRelevantContentFromHTML(String html, String url, boolean isGeneric) throws Exception
    {
        ...
        try {
            ...
            {
            	jobBoard = "monster.fr";
                res = getJobIdFromMonster(url);
            	res += getRelevantContentFromMonster(html);
        	}
    ...
    public static String getJobIdFromLeBonCoin(String url)
    {
        ...
        try
        {
            ...
            {   //https://www.leboncoin.fr/offres_d_emploi/1138951349.htm?ca=18_s
                //https://www.leboncoin.fr/offres_d_emploi/1145355271.htm
        ...
        try
        {
            ...
            if (url.indexOf("?jk=") >= 0) {
                if (url.indexOf("&amp;") >= 0)  // forme a1
                    oId = url.substring(url.indexOf("?jk=") + 4, url.indexOf("&amp;"));
                else    // forme a2
                    oId = url.substring(url.indexOf("?jk=") + 4);
            } else {
                String tmp = url;
                if (url.indexOf("?") >= 0) {   // b1
    ...
    public static String getRelevantContentFromMonster(String html) throws Exception
    {
        String res = "";

        if(html.indexOf("<div id=\"content")>-1)
        {
            res = html.substring(html.indexOf("<div id=\"content"));
            res = res.substring(0, res.indexOf("<footer"));
        }
        else
        {
            if(html.indexOf("CJT-mainbody")>=0)
            {
                res = html.substring(html.indexOf("<div id=\"CJT-mainbody"));
                res = res.substring(0,res.indexOf("<div id=\"ejb_sendJob"));
                res = res.substring(0,res.lastIndexOf("</div>"));
                res = res.substring(0,res.lastIndexOf("</div>"));
            }
            else if (html.indexOf("CJT-main")>=0)
            {
                res = html.substring(html.indexOf("<div id=\"CJT-main"));
                res = res.substring(0, res.indexOf("<div id=\"CJT-footer"));
            } 
            else 
            {
                res = html.substring(html.indexOf("<div id=\"jobwrapper"));
                res = res.substring(0, res.indexOf("<div id=\"bodycolbottom"));
            }
        }
    ...
    public static String getJobIdFromMonster(String url)
    {
        ...
        try
        {
            ...
            if(oId.indexOf("JobID=")>=0)
            {   // JobID= --> b
                oId = oId.substring(oId.indexOf("JobID=")+6,oId.indexOf("&amp;"));
            }
