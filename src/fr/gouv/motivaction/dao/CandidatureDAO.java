...
public class CandidatureDAO {
    ...
    /**
     * Modifie la date de candidature, ou de relance ou d'entretien d'une candidature
     * Appelée lorsqu'un evt est modifié uniquement
     * @param cand
     * @throws Exception
     */
    public static void updateDate(Candidature cand) throws Exception
    {
        Connection con = null;
        PreparedStatement pStmt = null;
        ResultSet rs = null;
        Timestamp dateEvt = null;
        ...
        try
        {
            ...
            // La candidature n'est chargée que partiellement, seule la date en lien avec l'evennement est enregistrée 
            if(cand.getDateCandidature() != 0) {
            	sql = "UPDATE candidatures SET dateCandidature = ? WHERE id = ?";
            	dateEvt = new Timestamp(cand.getDateCandidature());
            }
            else if(cand.getDateRelance() != 0) { 
            	sql = "UPDATE candidatures SET dateRelance = ? WHERE id = ?";
            	dateEvt = new Timestamp(cand.getDateRelance());
            }
            else if(cand.getDateEntretien() != 0) {
            	sql = "UPDATE candidatures SET dateEntretien = ? WHERE id = ?";
            	dateEvt = new Timestamp(cand.getDateEntretien());
            }

            pStmt = con.prepareStatement(sql);

           	pStmt.setTimestamp(1, dateEvt);
            pStmt.setLong(2, cand.getId());
    ...
    public static Candidature initCandidatureFromDB(ResultSet rs, boolean withDescription, boolean fromUserLog) throws Exception
    {
        ...
        if(rs.getTimestamp("dateCandidature") != null)
        	cand.setDateCandidature(rs.getTimestamp("dateCandidature").getTime());
        if(rs.getTimestamp("dateRelance") != null)
        	cand.setDateRelance(rs.getTimestamp("dateRelance").getTime());
        if(rs.getTimestamp("dateEntretien") != null)
        	cand.setDateEntretien(rs.getTimestamp("dateEntretien").getTime());
