...
MemoButtonImportRequest.prototype = {
    ...
    {
        console.log("initFrameMessageListener : ",this.importActive);
        window.addEventListener('message',function(e)
        {
            console.log("receiveParameters OK ",e.data, e, memoButtonImportRequest.importActive);
            ...
            {
                console.log("- keyboardClick reçu -",e.data, t.importActive);
    ...
    setButtonState : function(state)
    {
        ...
        if(!state)
        {
            this.setParentState("btn-memo-on","Enregistrer dans MEMO");
        }
        else if(state=="loading")
        {
            this.setParentState("btn-memo-loading","Enregistrement en cours");
        }
        else if(state=="error")
        {
            this.setParentState("btn-memo-off","Service momentanément indisponible");
        }
        else if(state=="success")
        {
            this.setParentState("btn-memo-off","Fiche enregistrée dans MEMO");
        }
        else if(state=="doublon")
        {
            this.setParentState("btn-memo-off","Vous avez déjà enregistré cette fiche dans MEMO");
        }
    ...
    setParentState : function(cls,tooltip)
    {
        var p = {type:"setState",id:this.frameId,tooltip:tooltip, className:cls};
        window.parent.postMessage(p,"*");
    },
