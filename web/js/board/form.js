...
CandidatureForm.prototype = {
    ...
    initCandidatureForm : function (candidature, onStart, doNotResetTunnel) {
        ...
        t.setCandidatureFormFieldValues(c);
    ...
    // initialise les champs de formulaire à  rien ou aux valeurs associées à la candidature en paramètre optionnel
    setCandidatureFormFieldValues : function(candidature)
    {
        var t = this, c = candidature;
    ...
    setDescriptionHeight: function () {
        setTimeout(function () {
            ...
            var t = lBR.board.form,
                h = t.description[0].scrollHeight,
                v = t.description.val();
            ...
            h = (h<80)?80:h;
            h = (h>2000)?2000:h;
            t.description.height(h);

            h = t.note[0].scrollHeight;
            v = t.note.val();
            if(!v)
                h = 80;
            h = (h<80)?80:h;
    ...
    resetErrorMessage: function () {
        ...
        $("#candidatureFormErrorMessages").html("").hide();
    ...
    autoSaveCandidature : function () {
        ...
        if (!t.logoUrl.val() && t.nomSociete.getValue())    // ajout d'une temporisation pour récupérer l'éventuel logo
            tO = 100;
    ...
    importOffreQuery: function (p, u, bt, async) {
        ...
        lBR.board.form.importFollowUpData = null;
    	...
    	$.ajax({
            ...
            success: function (response) {
                ...
                if(isInDomain(p, "monster") || isInDomain(p, "apec")) // import via json et non via html
                	html = response;
                else 
                	html = $sc(response);
                ...
                if (response == "error" || response == "expired" || (json && json.result == "error") || (json && json.result == "expired") )
                    lBR.board.form.manageImportError(u,bt);
                else {
                ...
                lBR.board.form.cleanUpAfterImport();
            },
            error: function (jqXHR, textStatus, errorThrown)
            {
                lBR.board.form.manageImportError(u,bt);
    ...
    {
        this.setOffreFieldDisabledState(false);
        this.board.setQuickImportDisabledState(false);
    },

    displayFormAfterSaveError : function()
    {
        this.hideTunnelSpinner();
        var fUD = this.importFollowUpData;
        this.showFormSaveError(fUD.u, fUD.bt, fUD.cand);
    },

    manageImportError : function(u,bt)
    {
        this.hideTunnelSpinner();
        ...
        if (u.indexOf('linkedin.com') >= 0)
            ...
            toastr['warning']("Cette offre n'est plus disponible, n'existe pas ou est bloquée", "");

        this.showFormParseError(u, bt);
    },
    ...
    showFormParseError: function (u, bt, cand) {
        ga('send', {
            hitType: 'event',
            eventCategory: 'Candidature',
            eventAction: 'import',
        ...
        this.showFormSaveError(u, bt, cand);
    },

    showFormSaveError: function (u, bt, cand) {

        if(this.board.selectedCandidature && !this.board.selectedCandidature.id)
            this.board.selectedCandidature = null;  // permet de ne pas afficher des éléments normalement réservés à l'édition dans le cas d'une nouvelle candidature

        this.initCandidatureForm();
        this.setCandidatureFormFieldValues(cand);
                ...
                if (form.tunnelType == CS.TYPES_CANDIDATURE.OFFRE && form.urlSource0.getValue() != "") {
                    ...
                    if (h != 1) { // On récupère les données saisies dans les écrans précédents du tunnel
                        if (form.tunnelType == CS.TYPES_CANDIDATURE.RESEAU) {
                            ...
                            form.nomCandidature.setValue(form.titrePosteTunnel.getValue());
                            form.numSiret.setValue(form.numSiret.getValue());   // ? pourquoi ?
                            form.registerChange(); // pr afficher la modal de sortie
                        }
    ...
    checkFormValues : function()
    {
        ...
        if(!res)
        {
            $("#candidatureFormErrorMessages").html("Le formulaire comporte des erreurs, veuillez corriger les champs en rouge.").show();
        }
    ...
    initTest : function(c, url) {
    	...
    	this.numSiret = new TextField({});
    	this.numSiret.field = $('<div></div>');
