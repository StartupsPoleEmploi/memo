...
Board.prototype = {
    ...
    archiveMode: 0,
    adviceWeight: {
        "": 0, "Conseil : supprimer": -1, "Conseil : postuler": 1,
    ...
    animateArchiveCandidature : function(el,elM,dur)
    {
        ...
        $(".marginMenu").toggleClass("glowAnimation");
    ...
    buildCandidatures : function()
    {
        ...
        setTimeout(function(){  // setimeout = hack pour permettre l'affichage du spinner
            ...
            if (ct == 0) {
            	// la page de présentation de MEMO est affichée si pas encore de candidatures enregistrées
            	t.displayStartButton(ct);
            } else {
    ...
    {
        var vidDiv = $("#videoFrameForm"), vid = $("#videoFrame")/*, prt = vid.parent().attr("id")*/;
        ...
        if(!memoVars.isVisitor && !ct && !this.archiveMode && lBR.loggedIn && !this.importOnStartupNotConsumed) // importOnStartupNotConsumed empêche de réinitialiser le formulaire lors d'un import on startup
        {
            ...
            /*if(prt!="videoFrameForm")
                lBR.moveVideo("videoFrameForm");*/
    ...
    buildCandidatureMenu : function(menu)
    {
        ...
        if(!t.archiveMode)
        {
            ...
            h+='<li id="'+arId+'" rel="'+id+'"><i class="fa fa-trash"></i> Supprimer</li>';
        }
        else
        {
