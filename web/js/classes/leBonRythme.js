...
leBonRythme.prototype = {
    ...
    init : function()
    {
        ...
        // gestion cas utilisateur PE Connecté (SSO)
        t.managePeConnectedUser();
    },
    ...
    logoutUser : function()
    {
        ...
        else
        {
            ...
            lBR.logEventToGA('event', 'Utilisateur', 'Déconnexion', '');
    ...
    managePeConnectedUser : function()
    {
        if(window.parent !== window.self)
        {
            if(window.self.location.href.indexOf("PEAMConnect=1")>-1)
            {
                var url = parent.memoVars.url;  // on conserve l'URL du parent pour les importOnStartup
                parent.memoVars = memoVars;
                parent.memoVars.url = url;
    ...
    saveUserSource : function()
    {
        ...
        if(
            ...
            ||
            (memoVars.user.source=="PEAM" && (((new Date()).getTime()-memoVars.user.creationTime)<120000) && getAccountSource() )
        )
        {
            if(!source)
                source = getAccountSource();

            p = "source=" + source +"&csrf="+$("#csrf").val();
            ...
            $.ajax({
                ...
                data: p,
                dataType: "json",
                success: function (response) { memoVars.user.source = source; },
                error : function(jqXHR, textStatus, errorThrown) {
    ...
    initUser : function()
    {
        const u = memoVars.user;

        memoVars.user.lastName = Url.decode(u.lastName);
        memoVars.user.firstName = Url.decode(u.firstName);
